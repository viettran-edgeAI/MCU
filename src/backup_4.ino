...
typedef enum rf_flag{
    ...
    OOB_DATA,                // out-of-bag data, used for OOB evaluation
    VALIDATION_DATA
}rf_flag;
...
class Rf_data {
  public:
    ...
    void loadData(bool reuse = false) {
        ...
        uint8_t treeIndex = 0;
        if(flag == SUBSET_DATA || flag == OOB_DATA) {
...
public:
    ...
    vector<Rf_data> OOB;          // OOB set for each tree
    vector<uint16_t> test_backup;       // vector contains all sampleIDs of test_data
    vector<uint16_t> valid_backup;      // vector contains all sampleIDss of validation_data 
    ...
    RandomForest(String baseFile, int numtree, int max_depth, int min_split){
        ...
        splitData(0.7, "train", "test",  "validation");
  ...
  private:
    ...
    void splitData(float trainRatio, const char* extension_1, const char* extension_2, const char* extension_3) {
        ...
        // Calculate remaining samples for test and validation (split equally)
        uint16_t remainingSamples = totalSamples - trainSize;
        uint16_t testSize = remainingSamples / 2;
        uint16_t validSize = remainingSamples - testSize;
        
        ChainedUnorderedSet<uint16_t> train_sampleIDs;
        ChainedUnorderedSet<uint16_t> test_sampleIDs;
        ChainedUnorderedSet<uint16_t> valid_sampleIDs;

        train_sampleIDs.reserve(trainSize);
        test_sampleIDs.reserve(testSize);
        valid_sampleIDs.reserve(validSize);

        // Clear and prepare backup vectors
        test_backup.clear();
        valid_backup.clear();
        test_backup.reserve(testSize);
        valid_backup.reserve(validSize);
        ...
        // Collect remaining samples (not in training set)
        vector<uint16_t> remainingSampleIDs;
        remainingSampleIDs.reserve(remainingSamples);
        for(uint16_t i = 0; i < totalSamples; ++i) {
            if (train_sampleIDs.find(i) == train_sampleIDs.end()) {
                remainingSampleIDs.push_back(i);
            }
        }

        // Randomly split remaining samples between test and validation
        ChainedUnorderedSet<uint16_t> selectedForTest;
        selectedForTest.reserve(testSize);
        
        while (selectedForTest.size() < testSize && selectedForTest.size() < remainingSampleIDs.size()) {
            uint16_t idx = static_cast<uint16_t>(esp_random() % remainingSampleIDs.size());
            selectedForTest.insert(remainingSampleIDs[idx]);
        }
        selectedForTest.fit();

        // Assign samples to test and validation sets
        for(uint16_t sampleId : remainingSampleIDs) {
            if (selectedForTest.find(sampleId) != selectedForTest.end()) {
                test_sampleIDs.insert(sampleId);
                test_backup.push_back(sampleId);
            } else {
                valid_sampleIDs.insert(sampleId);
                valid_backup.push_back(sampleId);
            }
        }
        ...
        Serial.printf("Number of samples in validation set: %d\n", validSize);
        ...
        validation_data.allSamples.reserve(validSize);
        // Copy validation samples
        for(auto sampleId : valid_sampleIDs){
        ...
        for (uint8_t i = 0; i < numSubData; i++) {
            ...
            for (uint16_t id : allSampleIds) {
                if (inBagSamples.find(id) == inBagSamples.end()) {
                    oob_data.allSamples[id] = data.allSamples[id]; // Add to OOB set
                }
            }
            oob_data.allSamples.fit();
            oob_data.releaseData();
            OOB.push_back(oob_data); // Store OOB set for this tree
        }
    ...
    // helper funcion : create train-backup from test_backup , valid_backup and numSamples
    vector<uint16_t> createTrainBackup() {
        vector<uint16_t> train_backup;
        train_backup.reserve(numSamples * 0.7); 
        if(test_backup.empty() || valid_backup.empty()) {
            Serial.println("❌ Test or validation backup is empty, cannot create train backup.");
            return train_backup;
        }
        // test_backup and valid_backup already sorted
        // Iterate through all sample IDs and add those not in test_backup or valid_backup
        uint16_t test_cursor = 0;
        uint16_t valid_cursor = 0;
        for(uint16_t i = 0; i < numSamples; i++) {
            // Check if i is in test_backup or valid_backup
            if (test_cursor < test_backup.size() && test_backup[test_cursor] == i) {
                test_cursor++;
            } else if (valid_cursor < valid_backup.size() && valid_backup[valid_cursor] == i) {
                valid_cursor++;
            } else {
                // i is not in either backup, add to train_backup
                train_backup.push_back(i);
            }
        }
        train_backup.sort(); // Ensure train_backup is sorted
        return train_backup;
    }
    ...
    void restore_data(rf_flag& data_flag, uint8_t treeIndex) {
        ...
        if(data_flag == TRAINING_DATA || data_flag == TESTING_DATA || data_flag == VALIDATION_DATA) {   

            Serial.println("Restoring data from backup...");
            ...
            switch(data_flag) {
                case TRAINING_DATA:
                    sampleID_bag = createTrainBackup(); // Use train_backup for training data
                    train_data.allSamples.clear();
                    train_data.allSamples.reserve(sampleID_bag.size());
                    train_data.isLoaded = true;
                    break;
                case TESTING_DATA:
                    sampleID_bag = test_backup; // Use test_backup for testing data
                    test_data.allSamples.clear();
                    test_data.allSamples.reserve(sampleID_bag.size());
                    test_data.isLoaded = true;
                    break;
                case VALIDATION_DATA:
                    sampleID_bag = valid_backup; // Use valid_backup for validation data
                    validation_data.allSamples.clear();
                    validation_data.allSamples.reserve(sampleID_bag.size());
                    validation_data.isLoaded = true;
                    break;
                default:
                    Serial.printf("❌ Invalid data flag: %d\n", data_flag);
                    baseFileHandle.close();
                    return;
            }
            auto data_pointer = (data_flag == TRAINING_DATA) ? &train_data : 
                                (data_flag == TESTING_DATA) ? &test_data : &validation_data;
           ...
           // All sampleIDs are sorted from smallest to largest, just browse baeFile once
            for(uint16_t sampleId : sampleID_bag) {
                ...
                data_pointer->allSamples[sampleId] = s;
            }
            data_pointer->allSamples.fit();
            baseFileHandle.close();
            Serial.printf("Data restored successfully for %s with %d samples.\n", 
                           (data_flag == TRAINING_DATA) ? "training" : 
                           (data_flag == TESTING_DATA) ? "testing" : "validation", 
            ...
            Rf_data& oob_data = OOB[treeIndex];
            // clear and reset old oob_data
            oob_data.allSamples.clear();
            if (oob_data.filename.length() > 0 && SPIFFS.exists(oob_data.filename.c_str())) {
                SPIFFS.remove(oob_data.filename.c_str());
            }
            oob_data.isLoaded = true;
            vector<uint16_t> train_backup = createTrainBackup(); // Create train_backup from test_backup and valid_backup
            ...
            // create OOB set for this tree
            for (uint16_t sampleId : train_backup) {
                if (subsetSampleIDs.find(sampleId) == subsetSampleIDs.end()) {
                    // Add to OOB set if not in subset
                    oob_data.allSamples[sampleId] = train_data.allSamples[sampleId];
                }
            }
            // release train_data immediately after done it's job 
            if(!preload){
            ...
            Serial.printf("OOB data for tree %d restored with %d samples.\n", treeIndex, oob_data.allSamples.size());

            Serial.printf("Restore successful !");
        }else if(data_flag == OOB_DATA){      
            // Restore OOB data for a specific tree
            if (treeIndex >= OOB.size()) {
                Serial.printf("❌ Invalid tree index: %d\n", treeIndex);
                return;
            }
            Rf_data& oob_data = OOB[treeIndex];
            if (oob_data.isLoaded) {
                Serial.printf("OOB data for tree %d already loaded, skipping restore.\n", treeIndex);
                return;
            }
            oob_data.allSamples.clear();
            oob_data.isLoaded = true;
            Rf_data& res_data = dataList[treeIndex]; // respectively of oob_data
            bool res_preload = res_data.isLoaded; // Check if res_data is already loaded
            if(res_data.isLoaded){
                res_data.loadData(true); // Load res_data if not already loaded
            }
            // get sampleIDs from res_data
            unordered_set<uint16_t> resSampleIDs;
            for (const auto& sample : res_data.allSamples) {
                resSampleIDs.insert(sample.first);
            }
            if(!res_preload){
                res_data.releaseData(true); // Release res_data after loading
            }

            bool train_preload = train_data.isLoaded; // Check if train_data is already loaded
            if(!train_data.isLoaded){
                train_data.loadData(true); // Load train_data if not already loaded
            }
            for (const auto& sample : train_data.allSamples) {
                if (resSampleIDs.find(sample.first) == resSampleIDs.end()) {
                    // If sample is not in res_data, add to OOB
                    oob_data.allSamples[sample.first] = sample.second;
                }
            }
            if(!train_preload){
                train_data.releaseData(true); // Release train_data after loading
            }
            // Clone samples from train_data based on train_data
            Serial.printf("OOB data for tree %d restored with %d samples.\n", treeIndex, oob_data.allSamples.size());
        }else{
    ...
    Tree_node* buildTree(Rf_data &a, uint8_t min_split, uint16_t max_depth) {
        ...
        for(uint8_t value : allFeaturesValue){                            
            ...
            if (sub_data.allSamples.empty()) {                // no samples left to split 
                ...
                Tree_node* childNode = buildTree(sub_data,min_split,max_depth-1); 
    ...
    // Predict class for a single sample using one tree
    uint8_t one_tree_predict(Tree_node* rootNode,  Rf_sample& s) {
    ...
    uint8_t predClassSample(Rf_sample& s){
        ...
        for(auto& node : root){
            ...
            uint8_t predict = one_tree_predict(node, s);
        ...
        // Check certainty unity_threshold 
    ...
    pair<float, float> get_training_evaluation_index(trainingFlags flag){
        ...
        // Load validation data for training evaluation
        bool validation_was_loaded = validation_data.isLoaded;
        if (!validation_was_loaded) {
            validation_data.loadData(true);
        }
        
        if (validation_data.allSamples.empty()) {
            Serial.println("❌ Validation data is empty, cannot evaluate.");
            return {0.0f, 0.0f};
        }
        
        // Load all trees once for ensemble evaluation
        bool trees_were_loaded = true;
        for (auto& tree : root) {
            if (!tree->isLoaded) {
                tree->loadTree();
                trees_were_loaded = false;
            }
        }
        
        // Initialize confusion matrices using stack arrays to avoid heap allocation
        uint16_t train_tp[numLabels] = {0}; 
        uint16_t train_fp[numLabels] = {0};
        uint16_t train_fn[numLabels] = {0};
        uint16_t train_correct = 0, train_total = 0;
        ...
        // Evaluate validation data using full ensemble
        Serial.println("Evaluating validation data with ensemble...");
        for (auto samplePair : validation_data.allSamples) {
            Rf_sample& sample = samplePair.second;
            uint8_t actual = sample.label;
            
            if (actual >= numLabels ) continue; // Safety check for array bounds
            
            // Use ensemble prediction (predClassSample)
            uint8_t pred = predClassSample(sample);
            
            if (pred >= numLabels) continue; // Skip uncertain predictions
        ...
        // Evaluate OOB data - use only trees that didn't see each sample
        Serial.println("Evaluating OOB data with proper ensemble...");
        for (size_t treeIndex = 0; treeIndex < OOB.size(); ++treeIndex) {
            Rf_data& oobData = OOB[treeIndex];
            
            // Load OOB data for this tree
            bool oob_was_loaded = oobData.isLoaded;
            if (!oob_was_loaded) {
                oobData.loadData(true);
            }
            
            // For each OOB sample, predict using ALL OTHER TREES (not including current tree)
            for (auto samplePair : oobData.allSamples) {
                Rf_sample& sample = samplePair.second;
                uint8_t actual = sample.label;
                
                if (actual >= numLabels) continue;
                
                // Make ensemble prediction using all trees EXCEPT the current one
                uint8_t predictCounts[numLabels] = {0}; // Stack array instead of unordered_map
                int16_t totalPredict = 0;
                
                for (size_t otherTreeIndex = 0; otherTreeIndex < root.size(); ++otherTreeIndex) {
                    if (otherTreeIndex == treeIndex) continue; // Skip current tree
                    
                    Tree_node* tree = root[otherTreeIndex];
                    uint8_t treePred = one_tree_predict(tree, sample);
                    
                    if (treePred < numLabels) {
                        predictCounts[treePred]++;
                        totalPredict++;
                    }
                }
                
                if (totalPredict == 0) continue;
                
                // Find most predicted class
                int16_t max = -1;
                uint8_t mostPredict = 255;
                for (uint8_t i = 0; i < numLabels; i++) {
                    if (predictCounts[i] > max) {
                        max = predictCounts[i];
                        mostPredict = i;
                    }
                }
                ...
                float certainty = static_cast<float>(max) / totalPredict;
                if (certainty < unity_threshold) continue;
                
                uint8_t pred = mostPredict;
                oob_total++;
            ...
            // Release OOB data if it wasn't originally loaded
            if (!oob_was_loaded) {
                oobData.releaseData(true);
            }
        }
        checkHeapFragmentation();
        // Release validation data if it wasn't originally loaded
        if (!validation_was_loaded) {
            validation_data.releaseData(true);
        }
        
        // Release trees if they weren't originally loaded
        if (!trees_were_loaded) {
            for (auto& tree : root) {
                tree->releaseTree();
            }
        }
        ...
        float validation_index = 0.0f;
        switch(flag) {
            case trainingFlags::ACCURACY:
                validation_index = (train_total == 0) ? 0.0f : static_cast<float>(train_correct) / train_total;
                break;
            case trainingFlags::PRECISSION: {
                float sum_prec = 0.0f;
                int valid_labels = 0;
                for (int l = 0; l < numLabels ; ++l) {
                    if (train_tp[l] + train_fp[l] > 0) {
                        sum_prec += static_cast<float>(train_tp[l]) / (train_tp[l] + train_fp[l]);
                        valid_labels++;
                    }
                }
                validation_index = (valid_labels == 0) ? 0.0f : sum_prec / valid_labels;
                break;
            }
            case trainingFlags::RECALL: {
                ...
                int valid_labels = 0;
                for (int l = 0; l < numLabels ; ++l) {
                    if (train_tp[l] + train_fn[l] > 0) {
                        sum_rec += static_cast<float>(train_tp[l]) / (train_tp[l] + train_fn[l]);
                        valid_labels++;
                    }
                }
                validation_index = (valid_labels == 0) ? 0.0f : sum_rec / valid_labels;
                break;
            }
            case trainingFlags::F1_SCORE: {
                ...
                int valid_labels = 0;
                for (int l = 0; l < numLabels ; ++l) {
                    if (train_tp[l] + train_fp[l] + train_fn[l] > 0) {
                        float prec = (train_tp[l] + train_fp[l] == 0) ? 0.0f : static_cast<float>(train_tp[l]) / (train_tp[l] + train_fp[l]);
                        float rec = (train_tp[l] + train_fn[l] == 0) ? 0.0f : static_cast<float>(train_tp[l]) / (train_tp[l] + train_fn[l]);
                        float f1 = (prec + rec == 0.0f) ? 0.0f : 2.0f * prec * rec / (prec + rec);
                        sum_f1 += f1;
                        valid_labels++;
                    }
                }
                validation_index = (valid_labels == 0) ? 0.0f : sum_f1 / valid_labels;
                break;
            }
            default:
                validation_index = (train_total == 0) ? 0.0f : static_cast<float>(train_correct) / train_total;
        ...
        // Calculate OOB metrics (same logic as above)
        float oob_evaluation_index = 0.0f;
        switch(flag) {
            case trainingFlags::ACCURACY:
                oob_evaluation_index = (oob_total == 0) ? 0.0f : static_cast<float>(oob_correct) / oob_total;
                break;
            case trainingFlags::PRECISSION: {
                float sum_prec = 0.0f;
                int valid_labels = 0;
                for (int l = 0; l < numLabels; ++l) {
                    if (oob_tp[l] + oob_fp[l] > 0) {
                        sum_prec += static_cast<float>(oob_tp[l]) / (oob_tp[l] + oob_fp[l]);
                        valid_labels++;
                    }
                }
                oob_evaluation_index = (valid_labels == 0) ? 0.0f : sum_prec / valid_labels;
                break;
            }
            case trainingFlags::RECALL: {
                float sum_rec = 0.0f;
                int valid_labels = 0;
                for (int l = 0; l < numLabels ; ++l) {
                    if (oob_tp[l] + oob_fn[l] > 0) {
                        sum_rec += static_cast<float>(oob_tp[l]) / (oob_tp[l] + oob_fn[l]);
                        valid_labels++;
                    }
                }
                oob_evaluation_index = (valid_labels == 0) ? 0.0f : sum_rec / valid_labels;
                break;
            }
            case trainingFlags::F1_SCORE: {
                float sum_f1 = 0.0f;
                int valid_labels = 0;
                for (int l = 0; l < numLabels; ++l) {
                    if (oob_tp[l] + oob_fp[l] + oob_fn[l] > 0) {
                        float prec = (oob_tp[l] + oob_fp[l] == 0) ? 0.0f : static_cast<float>(oob_tp[l]) / (oob_tp[l] + oob_fp[l]);
                        float rec = (oob_tp[l] + oob_fn[l] == 0) ? 0.0f : static_cast<float>(oob_tp[l]) / (oob_tp[l] + oob_fn[l]);
                        float f1 = (prec + rec == 0.0f) ? 0.0f : 2.0f * prec * rec / (prec + rec);
                        sum_f1 += f1;
                        valid_labels++;
                    }
                }
                oob_evaluation_index = (valid_labels == 0) ? 0.0f : sum_f1 / valid_labels;
                break;
            }
            default:
                oob_evaluation_index = (oob_total == 0) ? 0.0f : static_cast<float>(oob_correct) / oob_total;
        ...
        Serial.printf("📊 Training Results: Validation=%.3f, OOB=%.3f (from %d validation, %d OOB predictions)\n", 
                    validation_index, oob_evaluation_index, train_total, oob_total);
        
        return {validation_index, oob_evaluation_index};
    }
  ...
  public:
    ...
    void training(int epochs, bool early_stop = false) {
        ...
        float train_evaluation_index = initial_metrics.second;
        float oob_evaluation_index = initial_metrics.first;
        ...
        for ( uint16_t epoch = 0; epoch < epochs; epoch++) {
            ...
            train_evaluation_index = new_metrics.second;
            oob_evaluation_index = new_metrics.first;
