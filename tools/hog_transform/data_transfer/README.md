...
data_transfer/
├── pc_side/
│   └── transfer_hog_config.py    # Python script to send config from PC
└── esp32_side/
    └── hog_config_receiver.ino   # Arduino sketch to receive config on ESP32
```
...
void setup() {
    ...
    if (hog.loadConfigFromFile("/dataset_features/dataset_features_hogcfg.json")) {
        Serial.println("✅ HOG config loaded successfully!");
        
        // Now use hog.extract() with camera images
    } else {
        Serial.println("❌ Failed to load HOG config");
    }
}
```
...
📈 Successfully received 1 files

📁 HOG config files in file system:
   📋 /dataset_features/dataset_features_hogcfg.json (611 bytes) - HOG Configuration
   Total: 1 files (1 HOG configs)
   ✅ HOG configuration ready for use!
...
| Method | Pros | Cons |
|--------|------|------|
| **File Transfer** | ✅ Parameters guaranteed to match PC training<br>✅ Easy to update<br>✅ No code recompilation needed | ⚠️ Requires serial transfer step |
| **Hardcoded Config** | ✅ No transfer needed | ❌ Error-prone<br>❌ Must recompile to change<br>❌ Parameter mismatch risk |
