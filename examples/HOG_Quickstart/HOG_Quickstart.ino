 ...
 * 
 * Features:
 * ✓ Minimal 3-line setup OR config file loading
...
// Method 1: Use config file (recommended for production)
// #define USE_CONFIG_FILE "/dataset_features_hogcfg.json"

// Method 2: Use code-based setup (default, shown below)
...
void setup() {
    ...
    // Step 2: Setup HOG configuration
#ifdef USE_CONFIG_FILE
    // Method 1: Load configuration from file (generated by hog_processor tool)
    // This method is recommended for production as it ensures exact parameter matching
    // between training (PC) and inference (ESP32)
    
    // Initialize filesystem with selected storage mode
    if (!RF_FS_BEGIN(STORAGE_MODE)) {
        Serial.println("❌ Failed to mount filesystem!");
        Serial.println("Make sure your config file is uploaded to LittleFS or SD card");
        ...
        if (hog.loadConfigFromFile(USE_CONFIG_FILE)) {
            ...
            Serial.printf("   - Processing: %dx%d pixels\n", img_cfg.output_width, img_cfg.output_height);
        } else {
            Serial.printf("⚠️  Failed to load config file: %s\n", USE_CONFIG_FILE);
            Serial.println("Falling back to default configuration...");
            hog.setupForESP32CAM();
        }
    }
#else
    // Method 2: Code-based setup with optimal defaults (ONE LINE!)
 ...
 * 
 * Configuration File Method (recommended for production):
 * ------------------------------------------------------
 * 1. Generate config file on PC using hog_processor tool:
 ...
 * 
 * 2. Upload the _hogcfg.json file to ESP32 filesystem:
 *    - Using Arduino IDE: Tools → ESP32 Sketch Data Upload
 *    - Or manually copy to LittleFS/SD card
 * 
 * 3. Load configuration in your sketch:
 *    #include "Rf_file_manager.h"
 *    
 *    RF_FS_BEGIN();  // Mount filesystem
 *    hog.loadConfigFromFile("/model_name_hogcfg.json");
 *    
 *    // Now HOG uses exact same parameters as training!
 * 
 * Benefits of Config File Method:
 * ------------------------------
 * ✓ Guarantees identical parameters between training and inference
 * ✓ Easy to update without recompiling
 * ✓ Documents exact settings used for model training
 * ✓ Simplifies deployment across multiple devices
 * 
