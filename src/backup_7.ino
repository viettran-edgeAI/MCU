...
public:
  ...
  private:
    ...
    void restore_data(rf_flag& data_flag, uint8_t treeIndex) {
        ...
        if(data_flag == rf_flag::TRAINING_DATA || data_flag == rf_flag::TESTING_DATA || data_flag == rf_flag::VALIDATION_DATA) {   
    ...
    uint8_t findBestBinarySplit(ChainedUnorderedMap<Rf_sample>& allSamples, uint8_t featureID, bool use_Gini) {
        float maxGain = -1.0f;
        ...
        for (uint8_t threshold = 0; threshold <= 2; threshold++) {
            float gain = calculateBinarySplitGain(allSamples, featureID, threshold, use_Gini);
    ...
    float calculateBinarySplitGain(ChainedUnorderedMap<Rf_sample>& allSamples, uint8_t featureID, uint8_t threshold, bool use_Gini) {
        if (allSamples.size() == 0) return 0.0f;
        
        // Calculate base impurity
        float baseImpurity = use_Gini ? gini(allSamples) : entropy(allSamples);
        ...
        float leftImpurity = use_Gini ? gini(leftSamples) : entropy(leftSamples);
        float rightImpurity = use_Gini ? gini(rightSamples) : entropy(rightSamples);
    ...
    Tree_node* buildTree(Rf_data &a, uint8_t min_split, uint16_t max_depth, bool use_Gini) {
        ...
        float threshold = use_Gini ? 0.05f : 0.1f; // Gini gains are typically smaller
        ...
        uint8_t bestThreshold = findBestBinarySplit(a.allSamples, bestFeatureID, use_Gini);
  ...
  public:
    ...
    void training(int epochs, float combine_ratio = 0.5, bool early_stop = true){
        ...
        if(!this->use_validation){
            ...
            combine_ratio = 0.2f + (0.4f * size_factor) + (0.2f * label_balance);
        ...
        pair<float,float> eval1 = get_training_evaluation_index(trainFlag);
        rebuildForest(); // Rebuild to account for randomness
        pair<float,float> eval2 = get_training_evaluation_index(trainFlag);
        ...
        for(int epoch = 1; epoch <= epochs; epoch++){
            ...
            pair<float,float> evaluation_result = get_training_evaluation_index(trainFlag);
        ...
        if(current_combined_score < best_combined_score - min_improvement){
            ...
            pair<float,float> final_eval = get_training_evaluation_index(trainFlag);
...
void setup() {
    ...
    const char* filename = "/digit_data_8.csv"; // hard dataset : use_Gini = true/false | boostrap = true; 89/92% - sklearn : 90% (6,5)
