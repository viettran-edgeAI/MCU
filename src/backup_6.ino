...
// helper func: find best threshold for the given feature: 0 (0 | 1, 2, 3), 1 (0,1 | 2,3) or 2 (0,1,2 | 3)
    ...
    float get_training_evaluation_index(trainingFlags flag){
        ...
        // }
        uint8_t buffer_chunk = numSamples / 10;
        ...
        File trainFile = SPIFFS.open(train_data.filename, "r");
        if (!trainFile) {
            Serial.printf("❌ Failed to open training data file: %s\n", train_data.filename.c_str());
            return 0.0f;
        }
        if(train_samples_buffer.empty()){
            Serial.println("❌ No training samples found in the buffer!");
            trainFile.close();
            //  siwtch to plan B: load all sample from RAM
            ...
            end_pos = std::move(train_samples_buffer.size()); // Set end_pos to the size of all samples
        ...
        while(end_pos < numSamples){
            auto sample = train_samples_buffer.back();
            train_samples_buffer.pop_back();
            if(train_samples_buffer.empty() && end_pos < numSamples){
                start_pos += buffer_chunk;
        ...
        trainFile.close(); // Close the training data file
...
void setup() {
    ...
    const char* filename = "/digit_data_8.csv"; // hard dataset : useGini = true | boostrap = true; 78% - sklearn : 90% 

    RandomForest forest = RandomForest(filename,21, 4, 12);
    forest.useGini = true;
