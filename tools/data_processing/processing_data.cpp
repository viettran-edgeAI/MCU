...
class Rf_categorizer{
    ...
    mcu::vector<mcu::pair<std::string, uint8_t>> labelMapping; // original label -> normalized label mapping
...
public:
    ...
    Rf_categorizer(uint16_t numFeatures, uint8_t gpf, const mcu::vector<mcu::pair<std::string, uint8_t>>& labelMap){
        this->numFeatures = numFeatures;
        groupsPerFeature = gpf;
    ...
    void saveToCSV(const char* filename) const {
        ...
        // Header: numFeatures,groupsPerFeature,numLabels
        fout << numFeatures << "," << (int)groupsPerFeature << "," << labelMapping.size() << "\n";
        
        // Label mappings section: original_label,normalized_value
        for (const auto& mapping : labelMapping) {
            fout << "LABEL," << mapping.first << "," << static_cast<int>(mapping.second) << "\n";
        }
...
Rf_categorizer categorizeCSVFeatures(const char* inputFilePath, const char* outputFilePath, int groupsPerFeature, const mcu::vector<mcu::pair<std::string, uint8_t>>& labelMapping){
    ...
    Rf_categorizer ctg(n_feats, groupsPerFeature, labelMapping);
    for (int j = 0; j < n_feats; ++j) {
