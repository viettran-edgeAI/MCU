...
void print_bag(unordered_set<uint8_t> bag){
    cout << "bag: ";
...
vector<uint16_t> get_best_hashers(unordered_map<int,vector<uint16_t>> hashers_container){
...
vector<uint16_t> final_testing(unordered_map<int,vector<uint16_t>> hashers_container, int iterators_check = 1000){
...
int main(){
    int num_iterators = 4; 
    unordered_map<int,vector<uint16_t>> hashers_container;
    vector<uint16_t> best_hashers;
    ...
    unordered_map<int, float> collision_density;
