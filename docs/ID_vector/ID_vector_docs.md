...
**Memory allocation**: `(max_id - min_id + 1) × BitsPerValue` bits = `⌈((max_id - min_id + 1) × BitsPerValue) / 8⌉` bytes
...
#### Advance : enable min_id (example with min_id = 1000, default bpv = 1)
```
index:     0    1    2    3    4    5    6    7    8
...
vec.set_maxID(2000);                    // Range: 0-2000
vec.set_minID(500);                     // Range: 500-2000
vec.set_ID_range(1000, 3000);           // Range: 1000-3000
...
```cpp
// Get nth element (0-based, sorted order with repetitions)
index_type operator[](size_type index) const;

// Get largest ID (throws if empty)
...
```cpp
ID_vector<uint16_t, 1> vec1(0, 100);
ID_vector<uint16_t, 1> vec2(0, 100);
