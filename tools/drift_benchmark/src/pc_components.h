...
class Rf_tree {
  public:
    ...
    vector<vector<uint32_t>> node_label_counts; // Label counts for each node (primarily leaves)
    std::string filename;
    ...
    uint32_t predictSample(const Rf_sample& sample, uint8_t quant_bits) const {
        uint32_t leafIdx = findLeafIndex(sample, quant_bits);
        return (leafIdx < nodes.size()) ? nodes[leafIdx].getLabel() : 0;
    }

    uint32_t findLeafIndex(const Rf_sample& sample, uint8_t quant_bits) const {
        if (nodes.empty()) return 0;
        ...
        while (currentIndex < nodes.size() && !nodes[currentIndex].getIsLeaf() && safety_counter < nodes.size()) {
            safety_counter++;
    ...
    void updateNodeLabel(uint32_t nodeIndex, uint16_t label, uint16_t numLabels) {
        if (nodeIndex >= nodes.size()) return;
        
        if (node_label_counts.size() < nodes.size()) {
            node_label_counts.resize(nodes.size());
        }
        
        if (node_label_counts[nodeIndex].empty()) {
            node_label_counts[nodeIndex].resize(numLabels, 0);
            // Initialize with current label if it's a leaf
            if (nodes[nodeIndex].getIsLeaf()) {
                node_label_counts[nodeIndex][nodes[nodeIndex].getLabel()] = 1;
            }
        }
        
        if (label < numLabels) {
            node_label_counts[nodeIndex][label]++;
            
            // Update the node's label to the new majority
            uint32_t maxCount = 0;
            uint16_t majorityLabel = nodes[nodeIndex].getLabel();
            for (uint16_t i = 0; i < numLabels; ++i) {
                if (node_label_counts[nodeIndex][i] > maxCount) {
                    maxCount = node_label_counts[nodeIndex][i];
                    majorityLabel = i;
                }
            }
            nodes[nodeIndex].setLabel(majorityLabel);
        }
    }

    void purgeTree() {
        nodes.clear();
        node_label_counts.clear();
...
public:
    ...
    void loadCSVData(std::string csvFilename, uint16_t numFeatures, int max_sample = -1) {
        ...
        while (std::getline(file, line)) {
            if (max_sample != -1 && validSamples >= static_cast<uint32_t>(max_sample)) {
                break;
            }
            linesProcessed++;
...
struct Rf_config{
    ...
    bool metric_manually_set = false;
...
public:
    ...
    Rf_config (std::string init_path = config_path){
        ...
        // Helper lambda to extract value from JSON
        auto extract_value = [&](const std::string& key, bool is_string = false) -> std::string {
            size_t pos = content.find("\"" + key + "\"");
            if (pos == std::string::npos) return "";
            
            // Check for nested "value" object (legacy format)
            size_t value_pos = content.find("\"value\":", pos);
            
            // Heuristic: if "value": is found before the next key or end of object, use it
            size_t next_comma = content.find(",", pos);
            size_t next_brace = content.find("}", pos);
            size_t limit = std::min(next_comma == std::string::npos ? std::string::npos : next_comma, 
                                    next_brace == std::string::npos ? std::string::npos : next_brace);
            
            if (value_pos != std::string::npos && value_pos < limit) {
                pos = content.find(":", value_pos) + 1;
            } else {
        ...
        if (!(val = extract_value("num_trees")).empty()) num_trees = static_cast<uint16_t>(std::stoi(val));
        if (!(val = extract_value("quantization_coefficient")).empty()) quantization_coefficient = static_cast<uint16_t>(std::stoi(val));
        quantization_coefficient = QuantizationHelper::sanitizeBits(static_cast<int>(quantization_coefficient));
        
        if (!(val = extract_value("criterion", true)).empty()) use_gini = (val == "gini");
        if (!(val = extract_value("use_bootstrap")).empty()) use_bootstrap = (val == "true");
        
        if (!(val = extract_value("training_score", true)).empty()) {
            if (val == "oob_score" || val == "valid_score" || val == "k_fold_score") training_score = val;
        }

        if (!(val = extract_value("k_folds")).empty()) k_folds = static_cast<uint16_t>(std::stoi(val));
        if (!(val = extract_value("random_seed")).empty()) random_seed = static_cast<uint32_t>(std::stoul(val));
        
        // Metric score
        val = extract_value("metric", true);
        if (val.empty()) val = extract_value("metric_score", true);
        
        if (!val.empty() && val != "auto") {
            if (val == "accuracy" || val == "ACCURACY") { metric_score = Rf_metric_scores::ACCURACY; metric_manually_set = true; }
            else if (val == "precision" || val == "PRECISION") { metric_score = Rf_metric_scores::PRECISION; metric_manually_set = true; }
            else if (val == "recall" || val == "RECALL") { metric_score = Rf_metric_scores::RECALL; metric_manually_set = true; }
            else if (val == "f1" || val == "f1_score" || val == "F1_SCORE") { metric_score = Rf_metric_scores::F1_SCORE; metric_manually_set = true; }
            else {
                std::cout << "⚠️ Unknown metric: " << val << ". Using auto-detection." << std::endl;
            }
        }

        // Data path (special handling because it might be in dataset_path in drift_config)
        if (!(val = extract_value("data_path", true)).empty()) data_path = val;
        else if (!(val = extract_value("dataset_path", true)).empty()) data_path = val;
        ...
        size_t pos = content.find("\"split_ratio\"");
        if (pos != std::string::npos) {
            ...
            if (split_start != std::string::npos && split_end != std::string::npos) {
                ...
                auto extract_sub_value = [&](const std::string& key) -> std::string {
                    size_t p = split_section.find("\"" + key + "\"");
                    if (p == std::string::npos) return "";
                    p = split_section.find(":", p) + 1;
                    size_t e = split_section.find_first_of(",}", p);
                    std::string v = split_section.substr(p, e - p);
                    v.erase(0, v.find_first_not_of(" \t\r\n"));
                    v.erase(v.find_last_not_of(" \t\r\n") + 1);
                    return v;
                };

                if (!(val = extract_sub_value("train_ratio")).empty()) { json_train_ratio = std::stof(val); train_ratio = json_train_ratio; }
                if (!(val = extract_sub_value("test_ratio")).empty()) { json_test_ratio = std::stof(val); test_ratio = json_test_ratio; }
                if (!(val = extract_sub_value("valid_ratio")).empty()) { json_valid_ratio = std::stof(val); valid_ratio = json_valid_ratio; }
            }
        }

        if (!(val = extract_value("enable_retrain")).empty()) enable_retrain = (val == "true");
        if (!(val = extract_value("enable_auto_config")).empty()) enable_auto_config = (val == "true");
        if (!(val = extract_value("max_samples")).empty()) max_samples = static_cast<uint32_t>(std::stoul(val));

        // Check and extract min_split
        if (!(val = extract_value("min_split")).empty()) {
            min_split = static_cast<uint16_t>(std::stoi(val));
            overwrite[0] = true;
        }

        // Check and extract min_leaf
        if (!(val = extract_value("min_leaf")).empty()) {
            min_leaf = static_cast<uint16_t>(std::stoi(val));
            overwrite[1] = true;
        }

        // Check and extract max_depth
        if (!(val = extract_value("max_depth")).empty()) {
            max_depth = static_cast<uint16_t>(std::stoi(val));
            overwrite[2] = true;
        }
    ...
    void init(std::string data_path, int max_sample = -1){
        ...
        std::string line;
        while (std::getline(file, line)) {
            if (max_sample != -1 && numSamples >= static_cast<uint32_t>(max_sample)) {
                break;
            }
            // Trim whitespace
            line.erase(0, line.find_first_not_of(" \t\r\n"));
        ...
        if (labelCounts.size() > 0) {
            ...
            if (metric_manually_set) {
                std::cout << "🔧 Metric manually set to " << flagsToString(metric_score) << "\n";
            } else if (maxImbalanceRatio > 10.0f) {
