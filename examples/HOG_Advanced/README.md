...
- **Three Demo Modes**: Preset cycling, config file demo, and manual configuration
...
- LittleFS or SD card (optional, for config file demo)

## Demo Modes

Change `currentMode` in the code to try different modes:

### 1. PRESET_CYCLING (Default)
Cycles through 4 preset configurations automatically, showing performance comparison.

### 2. CONFIG_FILE_DEMO
Demonstrates loading configuration from a JSON file generated by the PC `hog_processor` tool. This is the **recommended production workflow**.

**Workflow**:
1. Generate config on PC: `./hog_processor hog_config.json`
2. Upload `model_name_hogcfg.json` to ESP32 filesystem
3. Load with: `hog.loadConfigFromFile("/model_name_hogcfg.json")`

**Benefits**:
- ✅ Exact parameter matching between training and inference
- ✅ Easy deployment updates without recompiling
- ✅ Documents training configuration
- ✅ Simplifies multi-device deployment

### 3. MANUAL_CONFIG
Shows programmatic configuration for custom parameter tuning.
...
void setup() {
    // Method 1: Custom configuration for your specific needs
    HOG_MCU::Config config;
    config.input_format = ImageProcessing::PixelFormat::GRAYSCALE;
    ...
    config.maintain_aspect_ratio = false;
...
### Production Workflow with Config Files
```cpp
#include "STL_MCU.h"
#include "hog_transform.h"
...
void setup() {
    ...
    if (RF_FS_BEGIN()) {
        // Method 2: Load from config file (recommended)
        if (hog.loadConfigFromFile("/my_model_hogcfg.json")) {
            Serial.println("✅ Config loaded!");
            Serial.printf("Model: %s\n", hog.getFeatureCsvPath().c_str());
            
            const auto& cfg = hog.getImageProcessingConfig();
            Serial.printf("Input: %dx%d → %dx%d\n", 
...
void loop() {
    ...
    // Features now match exactly what was used during training!
    const auto& features = hog.getFeatures();
