...
class Rf_data {
  public:
    ...
    sample_set loadData(vector<uint16_t>& sampleIDs_bag) {
...
public:
    ...
    vector<pair<Rf_data, OOB_set>> dataList; // vector of pairs: Rf_data and OOB set for each tree
    vector<uint16_t> train_backup;   // backup of training set sample IDs 
    vector<uint16_t> test_backup;    // backup of testing set sample IDs
    ...
    float boostrap_ratio = 0.632f; 
    trainingFlags trainFlag; // flags for training
    bool useGini = true;
    bool boostrap = false;
    ...
    RandomForest(String baseFile, int numtree, int max_depth, int min_split){
        ...
        splitData(train_ratio, "train", "test");
    ...
    void MakeForest(){
        ...
        for(uint8_t i = 0; i < numTree; i++){
            ...
            Tree_node* rootNode = buildTree(dataList[i].first, minSplit, maxDepth, useGini);
  ...
  private:
    ...
    void restore_data(rf_flag& data_flag, uint8_t treeIndex) {
        ...
        if(data_flag == rf_flag::TRAINING_DATA || data_flag == rf_flag::TESTING_DATA){   
            ...
            vector<uint16_t> *restore_backup;
            switch (data_flag) {
                case rf_flag::TRAINING_DATA:
                {
            ...
            vector<uint16_t> inBagSamplesVec;
            // create OOB set with samples not used in this tree
            ...
            subsetData.allSamples.fit(); // Fit the data structure
            subsetData.isLoaded = true; // Mark as loaded
            Serial.printf("Subset data for tree %d restored with %d samples.\n", treeIndex, subsetData.allSamples.size());
        }else{
    ...
    // helper func: find best threshold for the given feature: 0 (0 | 1, 2, 3), 1 (0,1 | 2,3) or 2 (0,1,2 | 3)
    uint8_t findBestBinarySplit(ChainedUnorderedMap<Rf_sample>& allSamples, uint8_t featureID, bool useGini) {
        float maxGain = -1.0f;
        uint8_t bestThreshold = 0;
        
        // Try all possible binary split thresholds: 0, 1, 2
        // threshold 0: (0) | (1,2,3)
        // threshold 1: (0,1) | (2,3) 
        // threshold 2: (0,1,2) | (3)
        for (uint8_t threshold = 0; threshold <= 2; threshold++) {
            float gain = calculateBinarySplitGain(allSamples, featureID, threshold, useGini);
    ...
    // Calculate gain for a specific binary split
    float calculateBinarySplitGain(ChainedUnorderedMap<Rf_sample>& allSamples, uint8_t featureID, uint8_t threshold, bool useGini) {
        if (allSamples.size() == 0) return 0.0f;
        
        // Calculate base impurity
        float baseImpurity = useGini ? gini(allSamples) : entropy(allSamples);
        
        // Split samples into left (<=threshold) and right (>threshold)
        ChainedUnorderedMap<Rf_sample> leftSamples, rightSamples;
        uint32_t totalSamples = allSamples.size();
        
        for (const auto& sample : allSamples) {
            if (sample.second.features[featureID] <= threshold) {
                leftSamples[sample.first] = sample.second;
            } else {
                rightSamples[sample.first] = sample.second;
            }
        }
        
        // Skip if one side is empty
        if (leftSamples.size() == 0 || rightSamples.size() == 0) {
            return 0.0f;
        }
        
        // Calculate weighted impurity after split
        float leftWeight = static_cast<float>(leftSamples.size()) / totalSamples;
        float rightWeight = static_cast<float>(rightSamples.size()) / totalSamples;
        
        float leftImpurity = useGini ? gini(leftSamples) : entropy(leftSamples);
        float rightImpurity = useGini ? gini(rightSamples) : entropy(rightSamples);
        
        float weightedImpurity = leftWeight * leftImpurity + rightWeight * rightImpurity;
        
        return baseImpurity - weightedImpurity;
    }
    ...
    Tree_node* buildTree(Rf_data &a, uint8_t min_split, uint16_t max_depth, bool useGini) {
        ...
        // Now find the best binary threshold for the chosen feature
        uint8_t bestThreshold = findBestBinarySplit(a.allSamples, bestFeatureID, useGini);
        ...
        for (const auto& sample : a.allSamples) {
            if (sample.second.features[bestFeatureID] <= bestThreshold) {
        ...
        if (!leftData.allSamples.empty()) {
            node->children.first = buildTree(leftData, min_split, max_depth - 1, useGini);
        } else {
            node->children.first = createLeafNode(a);
        }
        // RIGHT branch
        if (!rightData.allSamples.empty()) {
            node->children.second = buildTree(rightData, min_split, max_depth - 1, useGini);
        } else {
    ...
    float get_training_evaluation_index(trainingFlags flag){
        ...
        if(oob_total == 0){
            ...
            if(!train_data_preloaded) train_data.releaseData(true);
            return 0.0f;
        }
  ...
  public:
    ...
    // train model : using OOB (Out of Bag) samples to evaluate the model
        void training(int epochs, bool early_stop = true){
        ...
        const float min_improvement = 0.001f;
        
        // Training parameters limits
        const uint8_t min_minSplit = 3;
        const uint16_t max_maxDepth = 10;
        
        // Best state storage (memory-efficient)
        uint8_t best_minSplit = minSplit;
        uint16_t best_maxDepth = maxDepth;
        const char* best_state_prefix = "/best_tree_";
        
        // Parameter adjustment state
        bool can_adjust_minSplit = (minSplit > min_minSplit);
        bool can_adjust_maxDepth = (maxDepth < max_maxDepth);
        bool parameters_optimal = false;
        
        Serial.printf("Initial: minSplit=%d, maxDepth=%d\n", minSplit, maxDepth);
        
        // Get initial evaluation and save as best state
        current_evaluation_index = get_training_evaluation_index(trainFlag);
        best_evaluation_index = current_evaluation_index;
        saveBestState();
        Serial.printf("Initial score: %.4f (saved as best)\n", current_evaluation_index);
        ...
        for(int epoch = 1; epoch <= epochs; epoch++){
            ...
            float prev_score = current_evaluation_index;
            
            bool parameter_changed = false;
            
            // Try to improve parameters if not optimal yet
            if(!parameters_optimal){
                // Priority: reduce minSplit first (more splits = more complex trees)
                if(can_adjust_minSplit && minSplit > min_minSplit){
                    minSplit--;
                    parameter_changed = true;
                    Serial.printf("Reducing minSplit to %d\n", minSplit);
                    
                    if(minSplit == min_minSplit){
                        can_adjust_minSplit = false;
                        Serial.println("minSplit reached minimum");
                    }
                }
                // Then try increasing maxDepth
                else if(can_adjust_maxDepth && maxDepth < max_maxDepth){
                    maxDepth++;
                    parameter_changed = true;
                    Serial.printf("Increasing maxDepth to %d\n", maxDepth);
                    
                    if(maxDepth == max_maxDepth){
                        can_adjust_maxDepth = false;
                        Serial.println("maxDepth reached maximum");
                    }
                }
                else {
                    parameters_optimal = true;
                    Serial.println("Parameters reached optimal limits");
                }
            }
            
            if(parameter_changed){
                Serial.printf("New params: minSplit=%d, maxDepth=%d\n", minSplit, maxDepth);
            } else {
                Serial.println("Rebuilding with same parameters");
            }
            
            // Rebuild forest with current parameters
            Serial.printf("RAM before: %d bytes\n", ESP.getFreeHeap());
            rebuildForest();
            Serial.printf("RAM after: %d bytes\n", ESP.getFreeHeap());
            
            // Evaluate new performance
            current_evaluation_index = get_training_evaluation_index(trainFlag);
            float improvement = current_evaluation_index - prev_score;
            
            Serial.printf("Score: %.4f (change: %+.4f)\n", 
                        current_evaluation_index, improvement);
            
            // Check if this is the best performance so far
            if(current_evaluation_index > best_evaluation_index + min_improvement){
                // New best state - save it
                best_evaluation_index = current_evaluation_index;
                ...
                saveBestState();
                Serial.printf("✅ New best: %.4f (state saved)\n", best_evaluation_index);
            }
            else {
                no_improvement_count++;
                Serial.printf("⚠️ No improvement (%d/%d)\n", 
                            no_improvement_count, early_stop_patience);
                
                // If performance degraded significantly, restore best state
                if(current_evaluation_index < best_evaluation_index - min_improvement){
                    Serial.println("📥 Performance degraded, restoring best state...");
                    
                    // Restore best parameters
                    minSplit = best_minSplit;
                    maxDepth = best_maxDepth;
                    
                    // Restore best forest state
                    restoreBestState();
                    
                    current_evaluation_index = get_training_evaluation_index(trainFlag);
                    Serial.printf("Restored: minSplit=%d, maxDepth=%d, score=%.4f\n", 
                                minSplit, maxDepth, current_evaluation_index);
                    
                    // Stop parameter adjustments that led to degradation
                    if(parameter_changed){
                        if(prev_minSplit != minSplit){
                            can_adjust_minSplit = false;
                            Serial.println("Stopping minSplit adjustment");
                        }
                        if(prev_maxDepth != maxDepth){
                            can_adjust_maxDepth = false;
                            Serial.println("Stopping maxDepth adjustment");
                        }
                        
                        if(!can_adjust_minSplit && !can_adjust_maxDepth){
                            parameters_optimal = true;
                            Serial.println("All parameters now optimal");
                        }
                    }
                }
            }
            ...
            if(early_stop && no_improvement_count >= early_stop_patience){
                Serial.printf("🛑 Early stop after %d epochs\n", early_stop_patience);
                break;
            }
        ...
        // Final restoration to best state if current is not the best
        if(current_evaluation_index < best_evaluation_index - min_improvement){
            Serial.println("📥 Final restoration to best state...");
            minSplit = best_minSplit;
            maxDepth = best_maxDepth;
            restoreBestState();
            current_evaluation_index = best_evaluation_index;
        }
        
        // Cleanup best state files
        cleanupBestState();
        
        Serial.println("\n----------- Training completed ----------");
        Serial.printf("Final: minSplit=%d, maxDepth=%d\n", minSplit, maxDepth);
        Serial.printf("Best score: %.4f\n", best_evaluation_index);
        Serial.printf("Final score: %.4f\n", current_evaluation_index);
...
void setup() {
    ...
    // const char* filename = "/walker_fall.csv";    // medium dataset : useGini = false | boostrap = true; 92% - sklearn : 85%  (5,3)
    const char* filename = "/digit_data_8.csv"; // hard dataset : useGini = true/false | boostrap = true; 89/92% - sklearn : 90% (6,5)

    RandomForest forest = RandomForest(filename,12 ,6 ,6);
