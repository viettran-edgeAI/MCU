...
#include "eml/core/ml/common/eml_data.h"
#include "eml/core/ml/common/eml_tree.h"
#include "eml/core/ml/common/eml_matrix_score.h"
#include "eml/core/ml/common/eml_logger.h"
#include "eml/core/ml/common/eml_random.h"
#include "eml/core/models/random_forest/rf_config.h"
#include "eml/core/models/random_forest/rf_base.h"
#include "eml/core/models/random_forest/rf_node_predictor.h"
#include "eml/core/models/random_forest/rf_quantizer.h"
#include "eml/core/models/random_forest/rf_tree_container.h"
#include <cstdio>
#include <cmath>
...
namespace eml{
    ...
    class RandomForest{
        ...
        struct TrainingContext {
            ...
            Eml_data train_data;
            Eml_data test_data;
            Eml_data validation_data;
            Eml_random random_generator;
            bool build_model;
            bool data_prepared;
        ...
        Eml_tree_container forest_container;

        Rf_pending_data* pending_data = nullptr;
        Eml_data* base_data_stub = nullptr;
        ...
        inline Eml_data* ensure_base_data_stub(){
    #ifndef EML_STATIC_MODEL
            if(training_ctx){
    ...
    public:
        ...
        void cleanup_training_data(){
            ...
            // Purge all Eml_data to free resources
    ...
    private:
        ...
        bool prepare_forest_building_resource(){
            ...
            // data splitting
            vector<pair<float, Eml_data*>> dest;
            dest.reserve(3);
        ...
        bool splitData(Eml_data& source, vector<pair<float, Eml_data*>>& dest) {
            auto* ctx = training_ctx;
            if(!ctx){
                return false;
            }
        ...
        struct NodeStats {
            ...
            void analyzeSamples(const vector<rf_sample_type>& indices, rf_sample_type begin, rf_sample_type end,
                                    rf_label_type numLabels, const Eml_data& data) {
        ...
        vector<uint16_t> selectFeatureSubset(uint16_t num_selected_features, Eml_random& rng) {
        ...
        template<uint8_t bits>
        void buildTree(Eml_tree& tree, TreeSampleIDs<bits>& sampleIDs, vector<NodeToBuild>& queue_nodes) {
            auto* ctx = training_ctx;
            if(!ctx){
                return;
            }
        ...
        float get_oob_score(){
            ...
            Eml_matrix_score oob_scorer(config.num_labels, static_cast<uint8_t>(config.metric_score));
        ...
        float get_valid_score(){
            ...
            Eml_matrix_score valid_scorer(config.num_labels, static_cast<uint8_t>(config.metric_score));
        ...
        float get_cross_validation_score(){
            ...
            Eml_matrix_score scorer(config.num_labels, config.metric_score);
    ...
    public:
        ...
        void predict(const float* features, size_t length, rf_predict_result_t& result, bool get_original_label = true) {
            uint32_t start = rf_time_now(MICROSECONDS);
            ...
            if (__builtin_expect(config.enable_retrain, 1)) {
                ...
                if(auto* pd = ensure_pending_data()){
                    if(Eml_data* base_handle = ensure_base_data_stub()){
            ...
            if (__builtin_expect(get_original_label,1)){
                ...
                if (!quantizer.getOriginalLabelView(result.i_label, &labelPtr, &labelLen)) {
                    result.label[0] = '\0';
                    result.success = false;
                    result.prediction_time = rf_time_now(MICROSECONDS) - start;
                    return;
                }
    ...
    // ----------------------------------------getters---------------------------------------
        ...
        float get_last_n_inference_score(size_t num_inference, uint8_t flag) {
            ...
            Eml_matrix_score scorer(config.num_labels, flag);
        ...
        b_vector<b_vector<pair<rf_label_type, float>>> predict(Eml_data& data) {
            size_t start = logger.drop_anchor();
            ...
            // Initialize matrix score calculator
            Eml_matrix_score scorer(config.num_labels, 0xFF); // Use all flags for detailed metrics
        ...
        float precision(Eml_data& data) {
            // Create a temporary scorer to calculate precision
            Eml_matrix_score scorer(config.num_labels, 0x02); // PRECISION flag only
        ...
        float recall(Eml_data& data) {                                       
            Eml_matrix_score scorer(config.num_labels, 0x04); // RECALL flag only
        ...
        float f1_score(Eml_data& data) {
            Eml_matrix_score scorer(config.num_labels, 0x08); // F1_SCORE flag only
        ...
        float accuracy(Eml_data& data) {
            Eml_matrix_score scorer(config.num_labels, 0x01); // ACCURACY flag only
