 ...
 * ✓ Configuration file loading from filesystem
...
// Demo modes
enum DemoMode {
    PRESET_CYCLING,      // Cycle through preset configurations
    CONFIG_FILE_DEMO,    // Load and test configuration from file
    MANUAL_CONFIG        // Manual configuration testing
};

// Select demo mode (change this to try different modes)
DemoMode currentMode = PRESET_CYCLING;

// Configuration file path (used in CONFIG_FILE_DEMO mode)
...
void setupConfigFileDemo() {
    Serial.println("\n📁 Config File Demo Mode");
    Serial.println("========================");
    Serial.println("This mode demonstrates loading HOG configuration from a file");
    Serial.println("generated by the hog_processor tool on PC.\n");
    
    // Initialize filesystem with selected storage mode
    if (!RF_FS_BEGIN(STORAGE_MODE)) {
        Serial.println("❌ Failed to mount filesystem!");
        Serial.println("Make sure LittleFS or SD card is properly initialized");
    ...
    if (hog.loadConfigFromFile(CONFIG_FILE_PATH)) {
        ...
        Serial.println("📊 Loaded Configuration Details:");
        ...
        const auto& img_cfg = hog.getImageProcessingConfig();
        Serial.printf("   Input format: %d\n", (int)img_cfg.input_format);
        Serial.printf("   Input size: %dx%d\n", img_cfg.input_width, img_cfg.input_height);
        Serial.printf("   Output size: %dx%d\n", img_cfg.output_width, img_cfg.output_height);
        Serial.printf("   Resize method: %d\n", (int)img_cfg.resize_method);
        Serial.printf("   Maintain aspect: %s\n", img_cfg.maintain_aspect_ratio ? "Yes" : "No");
        Serial.printf("   JPEG quality: %d\n", img_cfg.jpeg_quality);
        
        Serial.println("\n💡 This configuration matches exactly what was used during");
        Serial.println("   training with the hog_processor tool on PC.");
        Serial.println("   This ensures consistent feature extraction!\n");
        
    } else {
        Serial.printf("❌ Failed to load config file: %s\n", CONFIG_FILE_PATH);
        Serial.println("\n🛠️  To use this demo mode:");
        Serial.println("   1. Run hog_processor on PC to generate config file");
        Serial.println("   2. Upload the _hogcfg.json file to ESP32 filesystem");
        Serial.println("   3. Update CONFIG_FILE_PATH in this sketch");
        Serial.println("\nFalling back to default configuration...\n");
        hog.setupForESP32CAM();
    }
}

void setupManualConfig() {
    Serial.println("\n⚙️  Manual Configuration Mode");
    Serial.println("============================");
    Serial.println("This mode demonstrates programmatic configuration.\n");
    
    // Create custom configuration
    HOG_MCU::Config custom;
    custom.input_format = ImageProcessing::PixelFormat::GRAYSCALE;
    custom.input_width = 320;
    custom.input_height = 240;
    custom.hog_img_width = 48;
    custom.hog_img_height = 48;
    custom.cell_size = 8;
    custom.block_size = 16;
    custom.block_stride = 8;
    custom.nbins = 6;
    custom.resize_method = ImageProcessing::ResizeMethod::BILINEAR;
    custom.maintain_aspect_ratio = false;
    custom.jpeg_quality = 80;
    
    hog.setConfig(custom);
    
    Serial.println("✅ Custom configuration applied");
    printConfigurationDetails(custom);
    Serial.println();
}

void setupPresetCycling() {
    Serial.println("\n🔄 Preset Cycling Mode");
    Serial.println("======================");
 ...
 * =================================
 * 
 * Method 1: Load from Config File (Recommended for Production)
 * ------------------------------------------------------------
 * This method ensures exact parameter matching between training and inference:
 * 
 ...
 *   
 *   Output files:
 *   - model_name.csv              (training features)
 *   - model_name_hogcfg.json      (ESP32 config)
 * 
 * Step 2 - Upload config to ESP32:
 *   Upload model_name_hogcfg.json to LittleFS or SD card
 * 
 * Step 3 - Load in your sketch:
 *   #include "Rf_file_manager.h"
 *   
 *   RF_FS_BEGIN();
 *   hog.loadConfigFromFile("/model_name_hogcfg.json");
 * 
 * Benefits:
 * ✓ Guarantees identical parameters between PC and ESP32
 ...
 * custom_config.input_format = ImageProcessing::PixelFormat::GRAYSCALE;
 * custom_config.input_width = 320;
 * custom_config.input_height = 240;
 ...
 * custom_config.resize_method = ImageProcessing::ResizeMethod::BILINEAR;
 * custom_config.maintain_aspect_ratio = false;
 * custom_config.jpeg_quality = 80;
 ...
 * {
 *   "model_name": "my_model",
 *   "feature_csv": "my_model.csv",
 *   "feature_file_name": "my_model.csv",
 *   "feature_length": 144,
 *   "preprocessing": {
 *     "grayscale": true,
 *     "normalize": true,
