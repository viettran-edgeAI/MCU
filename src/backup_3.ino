...
public:
  ...
  private:
    ...
    void restore_data(rf_flag& data_flag, uint8_t treeIndex) {
        ...
        if(data_flag == rf_flag::TRAINING_DATA || data_flag == rf_flag::TESTING_DATA){   
        ...
        }else if(data_flag == OOB_DATA){      // base data
            ...
            Serial.printf("Restoring OOB data for tree %d...\n", treeIndex);
            // Reconstruct OOB set from train_backup
            for (uint16_t sampleId : train_backup) {
                oob_data.allSamples[sampleId] = train_data.allSamples[sampleId];
            }
            if(!preload){
                train_data.releaseData(true); // Release train_data if it was loaded
            }
            oob_data.allSamples.fit();
    ...
    // Optimized training evaluation - one tree + its data at a time
    pair<float, float> get_training_evaluation_index(trainingFlags flag){
        ...
        bool trees_preloaded = true;
        for (auto& tree : root) {
            if (!tree->isLoaded) {
                tree->loadTree();
                trees_preloaded = false;
            }
        }
        
        // Initialize confusion matrices
        uint16_t* train_tp = new uint16_t[numLabels]();
        uint16_t* train_fp = new uint16_t[numLabels]();
        uint16_t* train_fn = new uint16_t[numLabels]();
        uint16_t train_correct = 0, train_total = 0;
        
        uint16_t* oob_tp = new uint16_t[numLabels]();
        uint16_t* oob_fp = new uint16_t[numLabels]();
        uint16_t* oob_fn = new uint16_t[numLabels]();
        uint16_t oob_correct = 0, oob_total = 0;
        
        // Evaluate training data using full ensemble
        Serial.println("Evaluating training data with ensemble...");
        ...
        // Release training data only if it wasn't preloaded
        if (!train_data_preloaded) {
            train_data.releaseData(true);
        }
        ...
        float train_evaluation_index = 0.0f;
        switch(flag) {
            case trainingFlags::ACCURACY:
                train_evaluation_index = (train_total == 0) ? 0.0f : static_cast<float>(train_correct) / train_total;
                break;
            case trainingFlags::PRECISSION: {
                ...
                train_evaluation_index = (valid_labels == 0) ? 0.0f : sum_prec / valid_labels;
                break;
            }
            case trainingFlags::RECALL: {
                ...
                train_evaluation_index = (valid_labels == 0) ? 0.0f : sum_rec / valid_labels;
                break;
            }
            case trainingFlags::F1_SCORE: {
                ...
                train_evaluation_index = (valid_labels == 0) ? 0.0f : sum_f1 / valid_labels;
                break;
            }
            default:
                train_evaluation_index = (train_total == 0) ? 0.0f : static_cast<float>(train_correct) / train_total;
        ...
        Serial.printf("📊 Training Results: Train=%.3f, OOB=%.3f (from %d train, %d OOB predictions)\n", 
                    train_evaluation_index, oob_evaluation_index, train_total, oob_total);
        // Clean up memory
        delete[] train_tp;
        delete[] train_fp;
        delete[] train_fn;
        ...
        return make_pair(train_evaluation_index, oob_evaluation_index);
    }
  ...
  public:
    ...
    void visual_result(Rf_data& testSet) {
        bool preload;
        loadForest(preload); // Ensure all trees are loaded before prediction
